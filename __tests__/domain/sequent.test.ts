...
test('disassembles (&&R): `X |- Y, A && B, Z ==> X |- A, Y, Z; X |- B, Y, Z`', () => {
    ...
    if (!actual) throw new Error('!actual is truthy');
    expect(actual[0].toString()).toBe('X |- A, Y, Z');
    if (!actual[1]) throw new Error('!actual[1] is truthy');
    expect(actual[1].toString()).toBe('X |- B, Y, Z');
});
...
test('disassembles (&&L): `X, A && B, Y |- Z ==> A, B, X, Y |- Z`', () => {
    ...
    const actual = disassemble(s);
    if (!actual) throw new Error('!actual is truthy');
    expect(actual[0].toString()).toBe('A, B, X, Y |- Z');
...
test('disassembles (||R): `X |- Y, A || B, Z ==> X |- A, B, Y, Z`', () => {
    ...
    const actual = disassemble(s);
    if (!actual) throw new Error('!actual is truthy');
    expect(actual[0].toString()).toBe('X |- A, B, Y, Z');
    expect(actual[1]).toBeUndefined();
});
...
test('disassembles (||L): `X, A || B, Y |- Z ==> A, X, Y |- Z; B, X, Y |- Z`', () => {
    ...
    const actual = disassemble(s);
    if (!actual) throw new Error('!actual is truthy');
    expect(actual[0].toString()).toBe('A, X, Y |- Z');
    if (!actual[1]) throw new Error('!actual[1] is truthy');
    expect(actual[1].toString()).toBe('B, X, Y |- Z');
});
...
test('disassembles (->R): `X |- Y, A -> B, Z ==> A, X |- B, Y, Z`', () => {
    ...
    const actual = disassemble(s);
    if (!actual) throw new Error('!actual is truthy');
    expect(actual[0].toString()).toBe('A, X |- B, Y, Z');
    expect(actual[1]).toBeUndefined();
});
...
test('disassembles (->L): `X, A -> B, Y |- Z ==> X, Y |- A, Z; B, X, Y |- Z`', () => {
    ...
    const actual = disassemble(s);
    if (!actual) throw new Error('!actual is truthy');
    expect(actual[0].toString()).toBe('X, Y |- A, Z');
    if (!actual[1]) throw new Error('!actual[1] is truthy');
    expect(actual[1].toString()).toBe('B, X, Y |- Z');
});
