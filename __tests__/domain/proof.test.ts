...
import { prove, toString } from "../../src/domain/proof";
...
test("Provable sequent: `A |- A`", () => {
  ...
  expect(toString(p.figure!)).toBe("A |- A");
});
...
test("Provable sequent: `|- A || !A`", () => {
  ...
  const p = prove(s);
  expect(p.provable).toBe(true);
  expect(toString(p.figure!)).toBe(["  A |- A  ", " -------- ", " |- A, !A ", "----------", "|- A || !A"].join("\n"));
});
